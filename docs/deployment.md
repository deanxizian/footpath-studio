...
构建和打包自动识别当前仓库：Vercel 使用 `VERCEL_GIT_REPO_OWNER` / `VERCEL_GIT_REPO_SLUG`，GitHub Actions 使用 `GITHUB_REPOSITORY`，本地使用 Git 的 `origin`（支持 GitHub HTTPS 和 SSH 地址）。Vercel 项目需开启 Automatically expose System Environment Variables；云端仓库信息缺失或无效时会报错，避免误用其他仓库的数据。无需维护来源配置文件。

默认构建从该仓库读取 `releases/latest/download/published-history.json`，不需要 GitHub API Token，也不依赖匿名 API 配额。最新月份必须保持 GitHub 的 Latest；以后发布软件版本时，应避免把软件 Release 设为 Latest。
...
发布前应校验全部附件。日常发布使用 [每日自动抓取](stryd-sync.md) 中的流程；账号密码仅保存在 Actions Secrets 中。只分析某个月时，可以单独下载月包；不要将多个月包直接解压到同一目录覆盖各自的 `version.json`。页面直接读取已发布索引，不提供手动导入入口。历史构建另外生成 assets 下的逐次平均轨迹摘要，供浏览器计算所选天数的三维基准；本地派生缓存位于 `.cache/trajectory-references/`。
